...
details <- function(my.origin, my.destination, column) {
  filtered <- flights %>% filter(origin == my.origin, dest == my.destination) %>% select_(column)
  return (filtered)
}

# Retireve the air_time column for flights from JFK to SEA

air_time_column <- details("JFK", "SEA", 'air_time')

# What was the average air time of those flights (in hours)?  

mean(air_time_column$air_time, na.rm = TRUE)
# What was the min/max average air time for the JFK to SEA flights?

max(air_time_column$air_time, na.rm = TRUE)
min(air_time_column$air_time, na.rm = TRUE)
